...
<div id="submit"  >
    ...
    <tbody>
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo '<td>';
            echo '<form method="post" action="delete_row_rollen.php" style="display:inline;">';
            echo '<input type="hidden" name="row_id" value="' . $row['idRollen'] . '">';
...
<tbody>
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo '<td>';
            echo '<form method="post" action="delete_row_categorie.php" style="display:inline;">';
            echo '<input type="hidden" name="row_id" value="' . $row['idcategorie'] . '">';
            echo '<button type="submit">Delete</button>';
            echo '</form>';
            echo '</td>';
            echo '</tr>';
...
<?php
   if (isset($_POST['submit5301'])) {
    ...
    $stmt->bind_param("ssss", $_POST['idLeverencie'], $_POST['DatumLevering'], $_POST['TijdLevering'], $_POST['idLeveranciers']);
    try {
        $stmt->execute();
   echo '<div style="display: flex; justify-content: center;">';
   echo "New record created successfully";
                ...
                if (isset($_POST['submit4'])) {
                    ...
                    if ($search !== null) {
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo '<td>';
                            echo '<form method="post" action="delete_row_levering.php" style="display:inline;">';
                            echo '<input type="hidden" name="row_id" value="' . $row['idLeverencie'] . '">';
                            echo '<button type="submit">Delete</button>';
                            echo '</form>';
                            echo '</td>';
                            echo '</tr>';
